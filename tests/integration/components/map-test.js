...
module("Integration | Component | map", function (hooks) {
  ...
  test("it renders a map image for the specified parameters", async function (assert) {
    await render(hbs`<Map @lat="37.7797"
      @lng="-122.4184"
      @zoom="10"
      @width="150"
      @height="120"/>`);
  ...
  test("it updates the `src` attribute when the arguments change", async function (assert) {
    ...
    assert.ok(
      ...
      "the src should inclued the lng, lat and zoom parameter"
    );
    assert.ok(
      img.src.includes("150x120@2x"),
      "the src shoudl include the width and height with @2x parameter"
    );
    this.setProperties({
      width: 300,
      height: 200,
    ...
    assert.ok(
      img.src.includes("-122.3321,47.60624"),
      "the src should inclued the lng, lat and zoom parameter"
    );
    assert.ok(
      img.src.includes("300x200@2x"),
